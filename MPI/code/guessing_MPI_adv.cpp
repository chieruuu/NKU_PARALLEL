...
{
    // 进阶任务模式下，Generate由主进程通过Generate_single调用
    // 这里只负责生成新PT
...
void PriorityQueue::Generate(PT pt)
{
    ...
    if (pt.content.size() == 1)
    {
        ...
        int start = mpi_rank * chunk + min(mpi_rank, rem);
        int end = start + chunk + (mpi_rank < rem ? 1 : 0);
...
// 进阶任务：单个PT生成口令，返回生成的口令列表（不修改成员变量）
vector<string> PriorityQueue::Generate_single(PT pt)
{
    ...
    if (pt.content.size() == 1)
    {
        ...
        for (int i = start; i < end; i += 1)
        {
            result.push_back(a->ordered_values[i]);
        }
    }
    else
    {
        ...
        for (int i = start; i < end; i += 1)
        {
            result.push_back(guess + a->ordered_values[i]);
        }
    }
