...
void MD5Hash_SIMD(string input[4], bit32 state[4][4])
{
    ...
    // 预处理，把 4 个密码分别填补到标准长度 (64 字节)
    for(int i = 0; i < 4; i++){
        paddedMessage[i] = StringProcess(input[i], &messageLength[i]);
    }

    // 初始化 4 个槽位的状态向量 (把原来的普通变量换成了向量类型)
    ...
    // 备份初始值
    uint32x4_t state0 = a, state1 = b, state2 = c, state3 = d;
    ...
    for (int i = 0; i < 16; ++i) {
        // 把 4 个密码同一位置的 32 位数据，打包进一个数组，然后加载成 128 位向量
        uint32_t data[4] = {ptr0[i], ptr1[i], ptr2[i], ptr3[i]};
        x[i] = vld1q_u32(data); 
    }
    ...
    // 加上初始值
    a = vaddq_u32(a, state0);
    b = vaddq_u32(b, state1);
    c = vaddq_u32(c, state2);
    d = vaddq_u32(d, state3);
